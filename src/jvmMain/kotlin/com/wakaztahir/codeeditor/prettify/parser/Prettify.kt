...
class Prettify {
    ...
    private val extensionMap by lazy {
        hashMapOf<String, LangProvider>().apply {
            ...
            registerExtensions(LangUrl.fileExtensions) { LangUrl() }
    ...
    inner class CreateSimpleLexer(
        ...
        fun decorate(job: Job) {
            ...
            while (ti < nTokens) {
                val token = tokens[ti]
                var style = PR_PLAIN
                var match: List<String> = emptyList()
                ...
                isEmbedded = style.length >= 5 && style.startsWith("lang-")
                ...
                if (!isEmbedded) {
                    if (match.size > 1) {
                        val underToken = match[1]
                        val afterUnderToken = token.subSequence(0, token.indexOf(underToken))
                        if (afterUnderToken.isNotEmpty()) {
                            decorations.add(basePos + tokenStart)
                            decorations.add(PR_PLAIN)
                            tokenStart += afterUnderToken.length
                        }

                        decorations.add(basePos + tokenStart)
                        decorations.add(style)
                        tokenStart += underToken.length

                        val posUnderToken = token.subSequence(token.indexOf(underToken) + underToken.length, token.length)
                        if (posUnderToken.isNotEmpty()) {
                            decorations.add(basePos + tokenStart)
                            decorations.add(PR_PLAIN)
                            tokenStart += posUnderToken.length
                        }
                    } else {
                        decorations.add(basePos + tokenStart)
                            decorations.add(style)
